...
doc: >-
  ...
  - changed base command /usr/bin/somatic_llr_filter.py to
  somatic_llr_filter2.py (embedded); original code throws error when DP is empty
  (but DP as a field exists), added additional check to handle empty values
  (None or [] in value), and also added additional checks for AD being empty
  ([]) or with less than two elements in array, in the new
  somatic_llr_filter2.py code
...
requirements:
  ...
  - class: InitialWorkDirRequirement
    listing:
      - entryname: somatic_llr_filter2.py
        entry: >-
          import argparse
          ...
          def calc_llr(normal_expect, tumor_expect, normal_ref, normal_var,
          tumor_ref, tumor_var):
          ...
          def get_llr(call, normal_ref, normal_var, tumor_ref, tumor_var,
          error_rate, heterozygous_expect,
          ...
          def make_call(normal_ref, normal_var, tumor_ref, tumor_var,
          error_rate, heterozygous_expect,
                        homozygous_expect, tumor_freq, tumor_purity, normal_contamination_rate, error_expect):
              ...
              for entry in vcf_reader:
                  ...
                  def getFormatField(sample_name, field_name):
                      if(sample_name in entry.call_for_sample and field_name in entry.call_for_sample[sample_name].data):
                          ...
                          if val: #added this to original code
                  ...
                  def incorrectAD(ad_val): #check to make sure AD has at least 2 elements in array; added this to original code
                      if not isinstance(ad_val, list):
                          return True
                      if len(ad_val)<2:
                          return True
                      return False
                  ...
                  for i in range(1,(len(alts)+1)):  #right now, this will only ever be one, due to above check.  Could be expanded to support multiple alleles - see above
                      if(ad_nrm == "NA" or incorrectAD(ad_nrm)):
                          normal_var = "NA"
                      else:
                          normal_var = ad_nrm[i]
                      if(ad_tum == "NA" or incorrectAD(ad_tum)):
                          tumor_var = "NA"
                      else:
                          tumor_var = ad_tum[i]
