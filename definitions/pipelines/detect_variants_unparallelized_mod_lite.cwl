...
doc: >-
  ...
  - this is created anew, as a lite version, to only use the
  mutect,varscan,strelka callers (not pindel)
...
label: Detect Variants workflow lite
$namespaces:
...
steps:
  - id: add_docm_variants
    in:
      - id: callers_vcf
        source: combine_variants_lite/combined_vcf
      - id: docm_vcf
  ...
  - id: combine_variants_lite
    ...
    out:
      - id: combined_vcf
    run: ../tools/combine_variants_lite.cwl
    label: CombineVariants (GATK 3.6)
    'sbg:x': 681.940673828125
    'sbg:y': 1894.34228515625
requirements:
